...
function EditorSocketIOServer (document, operations, docId) {
...
EditorSocketIOServer.prototype.addClient = function (socket) {
  ...
  socket
    ...
    .on('disconnect', function () {
      ...
      var emptyRoom = true
      if (Object.getOwnPropertyNames(self.users).length === 0) {
        self.emit('empty-room');
      }
      // if (socket.server.sockets.clients(self.docId).length === 0) {
      //   self.emit('empty-room');
      // }
    });
};
...
EditorSocketIOServer.prototype.onOperation = function (socket, revision, operation, cursor) {
  ...
  try {
    ...
    console.log("new operation: " + wrappedPrime.wrapped.toJSON());
